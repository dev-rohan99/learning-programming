    ...
    ChildView = ({ChildTitle, ChildSubTitle}) => {
        return (
            <View style={styles.main}>
                <View onPress={this.onClickItem.bind(ChildTitle)} style={styles.card2}>
    ...
    render() {
        return (
            <View>
                ...
                <FlatList horizontal={true} data={this.myData} renderItem={({item}) => <this.ChildView ChildTitle={item.title} ChildSubTitle={item.subtitle}/>} keyExtractor={item => item.id}/>

                <FlatList numColumns={2} horizontal={false} data={this.myData} renderItem={({item}) => <this.ChildView ChildTitle={item.title} ChildSubTitle={item.subtitle}/>} keyExtractor={item => item.id}/>
